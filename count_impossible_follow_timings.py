...
ts_incorrect = (
    df.join(nodes.select(["id", "date-created"]), left_on="to", right_on="id", how='left')
    .join(nodes.select(["id", "date-created"]), left_on="from", right_on="id", suffix="_from", how='left')
    .filter((pl.col("date_followed") < pl.col("date-created_from")) | (pl.col("date_followed") < pl.col("date-created")))
)
