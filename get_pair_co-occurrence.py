...
def main():
    ...
    cooccurrence_counts_list = list()
    cooccurrence_counts_list = [round(count) for count in cooccurrence_counter.values() if round(count) > 0]
    
    np.savetxt("data/pair_cooccurrence.csv.gz", cooccurrence_counts_list, fmt="%d")
